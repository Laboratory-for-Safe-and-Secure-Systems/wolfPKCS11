    ...
    switch (pMechanism->mechanism) {
        ...
        case CKM_ML_KEM_KEY_PAIR_GEN:
            ...
            *phPublicKey = *phPrivateKey = CK_INVALID_HANDLE;
            ...
            if (rv == CKR_OK) {
                ret = WP11_Kyber_GenerateKeyPair(pub, priv,
                                                 WP11_Session_GetSlot(session));
                if (ret != 0)
                    rv = CKR_FUNCTION_FAILED;
            }
...
CK_RV C_MessageEncryptInit(CK_SESSION_HANDLE hSession, CK_MECHANISM_PTR pMechanism,
                           CK_OBJECT_HANDLE hKey)
{
    if (!WP11_Library_IsInitialized())
        return CKR_CRYPTOKI_NOT_INITIALIZED;
                       ...
                       CK_BYTE_PTR pCiphertext, CK_ULONG_PTR pulCiphertextLen)
{
...
#if defined(HAVE_KYBER)
        case CKM_ML_KEM:
            ret = WP11_Kyber_Encapsulate(obj, &derivedKey, &keyLen,
    ...
    if (ret == 0) {
        ...
        if (rv == CKR_OK) {
            ...
            secretKeyLen[1] = keyLen;
            ret = WP11_Object_SetSecretKey(obj, secretKeyData, secretKeyLen);
            if (ret != 0)
                rv = CKR_FUNCTION_FAILED;
            if (ret == 0) {
                rv = AddObject(session, obj, pTemplate, ulAttributeCount,
...
#if defined(HAVE_KYBER)
        case CKM_ML_KEM:
            ret = WP11_Kyber_Decapsulate(obj, &derivedKey, &keyLen,
