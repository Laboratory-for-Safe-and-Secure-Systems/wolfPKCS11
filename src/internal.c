 ...
 *          0 on success.
 */
int WP11_Session_SetDilithiumParams(WP11_Session* session,
                                    CK_ML_DSA_PARAMS_PTR params,
                                    CK_ULONG paramsLen)
{
    int ret = 0;
    ...
    if (params != NULL) {
        ...
        if (params->pContextData != NULL && params->ulContextDataLen > 0) {
            dilithium->ctx = (byte*)XMALLOC(params->ulContextDataLen, NULL,
                                            DYNAMIC_TYPE_TMP_BUFFER);
            if (dilithium->ctx == NULL)
                ret = MEMORY_E;
            if (ret == 0) {
                XMEMCPY(dilithium->ctx, params->pContextData,
                        params->ulContextDataLen);
                dilithium->ctxSz = params->ulContextDataLen;
            }
        }
        else {
            dilithium->ctx = NULL;
        ...
        if (ret == 0) {
            if (params->phFlag == CK_TRUE)
                ret = wp11_hash_type(params->hash, &dilithium->preHashType);
            else
