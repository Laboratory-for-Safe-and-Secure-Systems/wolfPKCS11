...
static int wp11_Object_Decode_KyberKey(WP11_Object* object)
{
    ...
    if (object->objClass == CKO_PRIVATE_KEY) {
        ...
        if (ret == 0) {
            /* Decode Kyber private key. */
            ret = KyberKeyTryDecode(&object->data.kyberKey, WC_ML_KEM_512,
                                    der, len, object);
            if (ret != 0) {
                ret = KyberKeyTryDecode(&object->data.kyberKey, WC_ML_KEM_768,
                                        der, len, object);
            }
            if (ret != 0) {
                ret = KyberKeyTryDecode(&object->data.kyberKey, WC_ML_KEM_1024,
                                        der, len, object);
            }
            XMEMSET(der, 0, len);
        }
                            ...
                            CK_ULONG* len)
{
    ...
    switch (*params) {
        case CKP_ML_KEM_512:
            ret = wc_KyberKey_Init(WC_ML_KEM_512, key, NULL, object->slot->devId);
            break;
        case CKP_ML_KEM_768:
            ret = wc_KyberKey_Init(WC_ML_KEM_768, key, NULL, object->slot->devId);
            break;
        case CKP_ML_KEM_1024:
            ret = wc_KyberKey_Init(WC_ML_KEM_1024, key, NULL, object->slot->devId);
            break;
        default:
            ret = ASN_PARSE_E;
    }
...
#ifdef HAVE_KYBER
static int GetKyberParams(KyberKey* key, byte* data, CK_ULONG* len)
{
    int ret = 0;
    CK_ML_KEM_PARAMETER_SET_TYPE params;
...
static int GetKyberPublicKey(KyberKey* key, byte* data, CK_ULONG* len)
{
    int ret = 0;
    word32 dataLen = 0;
    ...
    if (data == NULL)
        *len = dataLen;
        ...
        ret = wc_KyberKey_EncodePublicKey(key, data, dataLen);
        if (ret == 0)
            *len = dataLen;
    }
...
static int GetKyberPrivateKey(KyberKey* key, byte* data, CK_ULONG* len)
{
    int ret = 0;
    word32 dataLen = 0;
    ...
    if (data == NULL)
        *len = dataLen;
        ...
        ret = wc_KyberKey_EncodePrivateKey(key, data, dataLen);
        if (ret == 0)
            *len = dataLen;
    }
...
/**
 * Get a Kyber object's data as an attribute.
 *
 * @param  object  [in]      Object object.
 * @param  type    [in]      Attribute type.
 * @param  data    [in]      Attribute data buffer.
 ...
 *          0 on success.
 */
static int KyberObject_GetAttr(WP11_Object* object, CK_ATTRIBUTE_TYPE type,
                               byte* data, CK_ULONG* len)
{
    int ret = 0;
    int noPriv = (((object->flag & WP11_FLAG_SENSITIVE) != 0) ||
    ...
    if (!(object->data.kyberKey.flags & KYBER_FLAG_PRIV_SET))
        noPriv = 1;
    if (!(object->data.kyberKey.flags & KYBER_FLAG_PUB_SET))
        noPub = 1;

    switch (type) {
        case CKA_PARAMETER_SET:
            ret = GetKyberParams(&object->data.kyberKey, data, len);
            break;
        case CKA_VALUE:
            if (object->objClass == CKO_PRIVATE_KEY) {
                if (noPriv)
                    *len = CK_UNAVAILABLE_INFORMATION;
                    ...
                    ret = GetKyberPrivateKey(&object->data.kyberKey, data, len);
            }
            else if (object->objClass == CKO_PUBLIC_KEY) {
                if (noPub)
                    *len = CK_UNAVAILABLE_INFORMATION;
                else
                    ret = GetKyberPublicKey(&object->data.kyberKey, data, len);
            }
            break;
        default:
            ret = NOT_AVAILABLE_E;
                        ...
                        CK_ULONG* len)
{
    ...
    switch (type) {
        ...
        #ifdef HAVE_KYBER
                        case CKK_ML_KEM:
                            ret = KyberObject_GetAttr(object, type, data, len);
                            break;
        #endif
        #ifndef NO_AES
                        case CKK_AES:
                               ...
                               WP11_Slot* slot)
{
    ...
    /* Both KyberKEy objects inside the pub and priv WP11_Objects are
     * already initialized and set to a proper level within
     * WP11_Object_SetKyberKey() based on the given parameter set. */

    /* Generate into the private key. */
    ret = Rng_New(&slot->token.rng, &slot->token.rngLock, &rng);
    if (ret == 0) {
        ret = wc_KyberKey_MakeKey(&priv->data.kyberKey, &rng);
        Rng_Free(&rng);
    }
    if (ret == 0) {
        ret = wc_KyberKey_PublicKeySize(&priv->data.kyberKey, &pubKeyLen);
    }
    if (ret == 0) {
        /* Allocate memory for the public key */
    ...
    if (ret == 0) {
        /* Export the public key */
        ret = wc_KyberKey_EncodePublicKey(&priv->data.kyberKey, pubKey, pubKeyLen);
    }
    if (ret == 0) {
        /* Copy the public part into public key. */
        ret = wc_KyberKey_DecodePublicKey(&pub->data.kyberKey, pubKey, pubKeyLen);
    }
    if (ret == 0) {
        priv->local = 1;
        pub->local = 1;
...
int WP11_Kyber_Encapsulate(WP11_Object* pub, unsigned char** sharedSecet, word32* ssLen,
                           CK_BYTE_PTR pCiphertext, CK_ULONG_PTR pulCiphertextLen)
{
    int ret;
    WC_RNG rng;
    KyberKey* kyberKey = &pub->data.kyberKey;
    word32 ctLen = 0;
    ...
    *sharedSecet = NULL;
    ...
    ret = wc_KyberKey_SharedSecretSize(kyberKey, ssLen);
    if (ret == 0) {
    ...
    if (ret == 0) {
        ret = wc_KyberKey_CipherTextSize(kyberKey, &ctLen);
    }
    ...
    if (ret == 0) {
        ret = wc_KyberKey_Encapsulate(kyberKey, pCiphertext, *sharedSecet, &rng);
        Rng_Free(&rng);
    }
...
int WP11_Kyber_Decapsulate(WP11_Object* priv, unsigned char** sharedSecet, word32* ssLen,
                           CK_BYTE_PTR pCiphertext, CK_ULONG ulCiphertextLen)
{
    int ret;
    KyberKey* kyberKey = &priv->data.kyberKey;
    ...
    *sharedSecet = NULL;
    ...
    if (ret == 0) {
        ret = wc_KyberKey_Decapsulate(kyberKey, *sharedSecet, pCiphertext, ulCiphertextLen);
    }
